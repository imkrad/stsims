<template>
    <b-modal v-model="showModal" header-class="p-3 bg-light" title="Update Barangay" class="v-modal-custom" modal-class="zoomIn" centered no-close-on-backdrop>
        <form class="customform">
            <BRow>
                <BCol lg="12">
                    <div>
                        <h6 class="mb-1">General Information</h6>
                        <p class="text-muted fs-11 mt-n1">Please ensure the barangay information is accurate and valid. </p>
                    </div>
                    <div>
                        <BRow class="g-3">
                            ...
                            <BCol lg="6" class="mt-0">
                                ...
                                <Multiselect :options="provinces" object v-model="filter.province" label="name" :searchable="true" placeholder="Select Province" />
                            </BCol>
                            <BCol lg="12" class="mt-0">
                                <InputLabel for="district" value="Municipality" :message="form.errors.municipality_code"/>
                                <Multiselect :options="municipalities" object v-model="form.municipality_code" label="name" :searchable="true" placeholder="Select Municipality" />
                            </BCol>
                        </BRow>
                    </div>    
                </BCol>
...
export default {
    ...
    methods: { 
        show(data){
            console.log(data.municipality.province.region);
            this.form.id = data.id;
            this.form.name = data.name;
            this.form.old_name = data.old_name;
            this.form.district = data.district;
            this.filter.region = this.transform(data.municipality.province.region);
            this.filter.province = this.transform(data.municipality.province);
            this.form.municipality_code = this.transform(data.municipality);
            this.showModal = true;
        },
        transform(data) {
            return {
