...
class InsightController extends Controller
{
    ...
    public function index(Request $request){
        ...
        switch($type){
            ...
            break;
            default : 
            return inertia('Modules/Operation/Insights/Index');
        }
    }
    ...
    public function home($request){
        ...
        $array = [
            ...
            'gender' => ScholarProfile::whereHas('scholar',function ($query){
            ...
            })->select(\DB::raw('count(*) as total'))->whereIn('sex',['F','M'])->groupBy('sex')->get(),
            'total' => Scholar::when($this->role, function ($query) {
                switch($this->role){
                    case 'PSTO Staff':
                        $query->whereHas('address',function ($query){
                            $query->where('province_code',$this->assigned);
            ...
            'ongoing' => Scholar::when($this->role, function ($query) {
                switch($this->role){
                    case 'PSTO Staff':
                        $query->whereHas('address',function ($query){
                            $query->where('province_code',$this->assigned);
            ...
            'graduated' => Scholar::when($this->role, function ($query) {
                switch($this->role){
                    case 'PSTO Staff':
                        $query->whereHas('address',function ($query){
                            $query->where('province_code',$this->assigned);
    ...
    public function courses($request){
        $sort = $request->sort;
        $data = ListCourse::withCount(['scholars as scholars_count' => function ($query) {
            $query->whereHas('scholar',function ($query){
                $query->when($this->role, function ($query) {
                    switch ($this->role) {
                        case 'PSTO Staff':
    ...
    public function location($request){
        ...
        })->when($request->region_code, function ($query, $region_code) {
            $query->where('region_code',$region_code);
        })->groupBy('province_code')->pluck('province_code');
        $provinces = LocationProvince::withCount(['scholars as scholars_count' => function ($query) {
            $query->whereHas('scholar',function ($query){
                $query->when($this->role, function ($query) {
                    switch ($this->role) {
                        case 'PSTO Staff':
        ...
        }])->whereIn('code',$provinces)->orderBy('scholars_count','DESC')->get();
        // $programs = ListProgram::where('is_sub',0)->get();
        $programs = ListDropdown::where('classification','Scholarship')->where('type','Category')->get();
        $array = []; $sums = []; $total = [];
    ...
    public function years(Request $request){
        ...
        foreach($programs as $program){
            ...
            for($year; $year <= $current_year; $year++){
                $years[] = $year;
                $data[] = ListProgram::where('program_id',$program->id)->withCount([
                'scholar', 
