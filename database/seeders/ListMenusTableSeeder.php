<?php

namespace Database\Seeders;

use Illuminate\Database\Seeder;

class ListMenusTableSeeder extends Seeder
{
    /**
     * Auto generated seeder file.
     *
     * @return void
     */
    public function run()
    {
        

        \DB::table('list_menus')->delete();
        
        \DB::table('list_menus')->insert(array (
            0 => 
            array (
                'id' => 1,
                'name' => 'Dashboard',
                'icon' => 'ri-apps-line',
                'route' => '/executive',
                'path' => 'Modules/Executive/Dashboard',
                'group' => 'Menu',
                'module' => 'Executive',
                'order' => 1,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            1 => 
            array (
                'id' => 2,
                'name' => 'User Management',
                'icon' => 'ri-team-fill ',
                'route' => '/executive/users',
                'path' => 'Modules/Executive/Users',
                'group' => 'Menu',
                'module' => 'Executive',
                'order' => 2,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            2 => 
            array (
                'id' => 3,
                'name' => 'Role Management',
                'icon' => 'ri-shield-check-line',
                'route' => '/executive/roles',
                'path' => 'Modules/Executive/Roles',
                'group' => 'Menu',
                'module' => 'Executive',
                'order' => 3,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            3 => 
            array (
                'id' => 4,
                'name' => 'Menu Management',
                'icon' => 'ri-list-check',
                'route' => '/executive/menus',
                'path' => 'Modules/Executive/Menus',
                'group' => 'Menu',
                'module' => 'Executive',
                'order' => 4,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            4 => 
            array (
                'id' => 5,
                'name' => 'Authentication Logs',
                'icon' => 'ri-time-fill',
                'route' => '/executive/authentications',
                'path' => 'Modules/Executive/Authentications',
                'group' => 'Menu',
                'module' => 'Executive',
                'order' => 5,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            5 => 
            array (
                'id' => 6,
                'name' => 'Activity Logs',
                'icon' => 'ri-history-line',
                'route' => '/executive/activities',
                'path' => 'Modules/Executive/Activities',
                'group' => 'Menu',
                'module' => 'Executive',
                'order' => 6,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            6 => 
            array (
                'id' => 7,
                'name' => 'Backup and Restore',
                'icon' => 'ri-server-line ',
                'route' => '/executive/backups',
                'path' => 'Modules/Executive/Backups',
                'group' => 'Menu',
                'module' => 'Executive',
                'order' => 7,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            7 => 
            array (
                'id' => 8,
                'name' => 'Dashboard',
                'icon' => 'ri-apps-line',
                'route' => '/references',
                'path' => 'Modules/References/Dashboard',
                'group' => 'Important',
                'module' => 'Reference',
                'order' => 1,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            8 => 
            array (
                'id' => 9,
                'name' => 'Agencies',
                'icon' => 'ri-hotel-fill',
                'route' => '/references/agencies',
                'path' => 'Modules/References/Agencies',
                'group' => 'Menu',
                'module' => 'Reference',
                'order' => 2,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            9 => 
            array (
                'id' => 10,
                'name' => 'Privileges',
                'icon' => 'ri-hand-coin-fill',
                'route' => '/references/privileges',
                'path' => 'Modules/References/Privileges',
                'group' => 'Menu',
                'module' => 'Reference',
                'order' => 3,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            10 => 
            array (
                'id' => 11,
                'name' => 'Programs',
                'icon' => 'ri-bill-fill',
                'route' => '/references/programs',
                'path' => 'Modules/References/Programs',
                'group' => 'Menu',
                'module' => 'Reference',
                'order' => 4,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            11 => 
            array (
                'id' => 12,
                'name' => 'Statuses',
                'icon' => 'ri-pages-fill',
                'route' => '/references/statuses',
                'path' => 'Modules/References/Statuses',
                'group' => 'Menu',
                'module' => 'Reference',
                'order' => 5,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            12 => 
            array (
                'id' => 13,
                'name' => 'Locations',
                'icon' => 'ri-earth-fill',
                'route' => NULL,
                'path' => 'Modules/References/Locations',
                'group' => 'List',
                'module' => 'Reference',
                'order' => 6,
                'has_child' => 1,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            13 => 
            array (
                'id' => 14,
                'name' => 'Regions',
                'icon' => NULL,
                'route' => '/references/regions',
                'path' => '/references/regions',
                'group' => 'Locations',
                'module' => 'Reference',
                'order' => 1,
                'has_child' => 0,
                'is_mother' => 0,
                'is_active' => 1,
            ),
            14 => 
            array (
                'id' => 15,
                'name' => 'Provinces',
                'icon' => NULL,
                'route' => '/references/provinces',
                'path' => '/references/provinces',
                'group' => 'Locations',
                'module' => 'Reference',
                'order' => 2,
                'has_child' => 0,
                'is_mother' => 0,
                'is_active' => 1,
            ),
            15 => 
            array (
                'id' => 16,
                'name' => 'Municipalities',
                'icon' => NULL,
                'route' => '/references/municipalities',
                'path' => '/references/municipalities',
                'group' => 'Locations',
                'module' => 'Reference',
                'order' => 3,
                'has_child' => 0,
                'is_mother' => 0,
                'is_active' => 1,
            ),
            16 => 
            array (
                'id' => 17,
                'name' => 'Barangays',
                'icon' => NULL,
                'route' => '/references/barangays',
                'path' => '/references/barangays',
                'group' => 'Locations',
                'module' => 'Reference',
                'order' => 4,
                'has_child' => 0,
                'is_mother' => 0,
                'is_active' => 1,
            ),
            17 => 
            array (
                'id' => 18,
                'name' => 'Dropdowns',
                'icon' => 'ri-list-unordered',
                'route' => NULL,
                'path' => 'Modules/References/Dropdowns',
                'group' => 'List',
                'module' => 'Reference',
                'order' => 7,
                'has_child' => 1,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            18 => 
            array (
                'id' => 19,
                'name' => 'Schools',
                'icon' => NULL,
                'route' => '/references/schools',
                'path' => '/references/schools',
                'group' => 'Dropdowns',
                'module' => 'Reference',
                'order' => 1,
                'has_child' => 0,
                'is_mother' => 0,
                'is_active' => 1,
            ),
            19 => 
            array (
                'id' => 20,
                'name' => 'Courses',
                'icon' => NULL,
                'route' => '/references/courses',
                'path' => '/references/courses',
                'group' => 'Dropdowns',
                'module' => 'Reference',
                'order' => 2,
                'has_child' => 0,
                'is_mother' => 0,
                'is_active' => 1,
            ),
            20 => 
            array (
                'id' => 21,
                'name' => 'Category',
                'icon' => NULL,
                'route' => '/references/categories',
                'path' => '/references/categories',
                'group' => 'Dropdowns',
                'module' => 'Reference',
                'order' => 3,
                'has_child' => 0,
                'is_mother' => 0,
                'is_active' => 1,
            ),
            21 => 
            array (
                'id' => 22,
                'name' => 'Schools',
                'icon' => 'ri-building-2-fill',
                'route' => '/references/schools',
                'path' => 'Modules/References/Schools',
                'group' => 'Important',
                'module' => 'Reference',
                'order' => 1,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 0,
            ),
            22 => 
            array (
                'id' => 23,
                'name' => 'Courses',
                'icon' => 'ri-list-unordered',
                'route' => '/references/courses',
                'path' => 'Modules/References/Courses',
                'group' => 'Important',
                'module' => 'Reference',
                'order' => 1,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 0,
            ),
            23 => 
            array (
                'id' => 24,
                'name' => 'Dashboard',
                'icon' => 'ri-apps-line',
                'route' => '/management',
                'path' => 'Modules/Management/Dashboard',
                'group' => 'Scholar',
                'module' => 'Management',
                'order' => 1,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            24 => 
            array (
                'id' => 25,
                'name' => 'Schools',
                'icon' => 'ri-building-2-fill',
                'route' => '/management/schools',
                'path' => 'Modules/Management/Schools',
                'group' => 'Menu',
                'module' => 'Management',
                'order' => 4,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            25 => 
            array (
                'id' => 26,
                'name' => 'Courses',
                'icon' => 'ri-list-unordered',
                'route' => '/management/courses',
                'path' => 'Modules/Management/Courses',
                'group' => 'Menu',
                'module' => 'Management',
                'order' => 6,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            26 => 
            array (
                'id' => 27,
                'name' => 'Certifications',
                'icon' => ' ri-file-text-fill',
                'route' => '/management/certifications',
                'path' => 'Modules/Management/Cetifications',
                'group' => 'Menu',
                'module' => 'Management',
                'order' => 7,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            27 => 
            array (
                'id' => 28,
                'name' => 'Campuses',
                'icon' => 'ri-hotel-fill',
                'route' => '/management/campuses',
                'path' => 'Modules/Management/Campuses',
                'group' => 'Menu',
                'module' => 'Management',
                'order' => 5,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            28 => 
            array (
                'id' => 29,
                'name' => 'Qualifiers',
                'icon' => 'ri-group-2-fill',
                'route' => '/management/qualifiers',
                'path' => 'Modules/Management/Qualifiers',
                'group' => 'Scholar',
                'module' => 'Management',
                'order' => 4,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            29 => 
            array (
                'id' => 30,
                'name' => 'Scholars',
                'icon' => 'ri-team-fill',
                'route' => '/management/scholars',
                'path' => 'Modules/Management/Scholars',
                'group' => 'Scholar',
                'module' => 'Management',
                'order' => 5,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            30 => 
            array (
                'id' => 31,
                'name' => 'Dashboard',
                'icon' => 'ri-apps-line',
                'route' => '/dashboard',
                'path' => 'Modules/Operation/Dashboard',
                'group' => 'Menu',
                'module' => 'Operation',
                'order' => 1,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            31 => 
            array (
                'id' => 32,
                'name' => 'Qualifiers',
                'icon' => 'ri-group-2-fill',
                'route' => '/qualifiers',
                'path' => 'Modules/Operation/Qualifiers',
                'group' => 'Menu1',
                'module' => 'Operation',
                'order' => 4,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            32 => 
            array (
                'id' => 33,
                'name' => 'Scholars',
                'icon' => 'ri-team-fill',
                'route' => '/scholars',
                'path' => 'Modules/Operation/Scholars',
                'group' => 'Menu1',
                'module' => 'Operation',
                'order' => 5,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            33 => 
            array (
                'id' => 34,
                'name' => 'Schools',
                'icon' => 'ri-building-2-fill',
                'route' => '/schools',
                'path' => 'Modules/Operation/Schools',
                'group' => 'Menu1',
                'module' => 'Operation',
                'order' => 3,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            34 => 
            array (
                'id' => 35,
                'name' => 'Enrollments',
                'icon' => 'ri-pages-line',
                'route' => '/enrollments',
                'path' => 'Modules/Operation/Enrollments',
                'group' => 'Menu2',
                'module' => 'Operation',
                'order' => 6,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            35 => 
            array (
                'id' => 36,
                'name' => 'Financial Benefits',
                'icon' => 'ri-wallet-3-fill',
                'route' => '/benefits',
                'path' => 'Modules/Operation/Benefits',
                'group' => 'Menu2',
                'module' => 'Operation',
                'order' => 7,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            36 => 
            array (
                'id' => 37,
                'name' => 'Monitoring',
                'icon' => 'bx bxs-webcam',
                'route' => '/monitoring',
                'path' => 'Modules/Operation/Monitoring',
                'group' => 'Menu2',
                'module' => 'Operation',
                'order' => 2,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            37 => 
            array (
                'id' => 38,
                'name' => 'Insights',
                'icon' => 'ri-line-chart-fill',
                'route' => '/insights',
                'path' => 'Modules/Operation/Insights',
                'group' => 'Menu3',
                'module' => 'Operation',
                'order' => 3,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            38 => 
            array (
                'id' => 39,
                'name' => 'Reports',
                'icon' => ' ri-file-text-fill',
                'route' => '/reports',
                'path' => 'Modules/Operation/Reports',
                'group' => 'Menu3',
                'module' => 'Operation',
                'order' => 2,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
            39 => 
            array (
                'id' => 40,
                'name' => 'Others',
                'icon' => 'ri-settings-4-fill',
                'route' => '/others',
                'path' => 'Modules/Operation/Others',
                'group' => 'Menu3',
                'module' => 'Operation',
                'order' => 4,
                'has_child' => 0,
                'is_mother' => 1,
                'is_active' => 1,
            ),
        ));

        
    }
}